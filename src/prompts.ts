...
export const PROMPT_TEMPLATES: Record<string, { systemInstruction: string; userPrompt: (...args: any[]) => string }> = {
  ...
  cluster_planner: {
    systemInstruction: "You are a SOTA content strategist specializing in pillar-cluster content architecture. Create comprehensive content plans that establish topical authority. Return valid JSON only.",
    userPrompt: (topic: string, existingPages: string | null, serpData: string | null) => {
      return `Create a content cluster plan for: "${topic}"
...
Generate 8-12 cluster titles that comprehensively cover the topic.`;
  ...
  json_repair: {
    systemInstruction: "You are a JSON repair specialist. Fix malformed JSON and return ONLY valid JSON. Do not add explanations. Return the repaired JSON only.",
    userPrompt: (brokenJson: string) => {
      return `Fix this malformed JSON and return ONLY the corrected JSON:
...
Be specific and prioritize by impact.`,
    userPrompt: (crawledContent: string, pageUrl: string, pageTitle: string) => {
      return `Analyze this webpage content for optimization opportunities:

**URL:** ${pageUrl}
**Title:** ${pageTitle}

**Content:**
${crawledContent ? crawledContent.substring(0, 12000) : "No content available"}
...
Be specific and actionable. Prioritize issues by impact.`;
  ...
  semantic_keyword_generator: {
    systemInstruction: "You are a semantic SEO expert. Generate comprehensive keyword clusters for topical authority. Return valid JSON only.",
    userPrompt: (primaryKeyword: string, serpData: string | null, location: string | null) => {
      return `Generate semantic keywords for: "${primaryKeyword}"
...
Return clean HTML5 with inline styles. No markdown.`,
    userPrompt: (articlePlan: string, semanticKeywords: string, competitorGaps: string, existingPages: string, neuronData: string | null, recentNews: string | null) => {
      return `## ARTICLE PLAN
${articlePlan}
...
Return complete, beautifully styled HTML5 content.`,
    userPrompt: (existingContent: string, pageTitle: string, semanticKeywords: string, existingPages: string, competitorGaps: string | null) => {
...
3. Entity Injection - "smartwatch" becomes "Apple Watch Series 10"
...
export const getPromptTemplate = (key: string) => {
...
export const buildPrompt = (key: string, ...args: any[]): { system: string; user: string } | null => {
